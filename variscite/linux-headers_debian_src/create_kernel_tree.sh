...
function create_debian_kernel_headers_package()
{
	...
	do
		...
		cp $i $dst
		echo $dst $i | sed -e 's/\/[^\/]*$//' >>  ${LINUX_COMMON_ARM64_INSTALL_FILE_PATH_NAME}
	done

	echo "kernel/Makefile /usr/src/linux-headers-5.4.3-arm64" >> ${LINUX_COMMON_ARM64_INSTALL_FILE_PATH_NAME}
	echo "kernel/.config /usr/src/linux-headers-5.4.3-arm64" >> ${LINUX_COMMON_ARM64_INSTALL_FILE_PATH_NAME}
	echo "kernel/Module.symvers /usr/src/linux-headers-5.4.3-arm64" >> ${LINUX_COMMON_ARM64_INSTALL_FILE_PATH_NAME}
